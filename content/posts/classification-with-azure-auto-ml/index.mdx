...
Today’s post covers how to use Azure AutoML to solve a classification problem. We will cover the following:
...
This is a very straightforward process and only requires a few minutes of configuration – most of your time will be spent waiting for the model to train itself while you are free to go for coffee 🙂

## 1. Problem statement and data assumptions

We will be using Microsoft’s Adventure Works Cycles data set to predict whether a customer will purchase a new bike. We have a CSV file with >16,000 rows to train on, which looks as follows:
...
This will take a couple of minutes to set up, but once it has completed, your workspace is ready to go – easy!
...
Give your experiment a name, and select a compute for it. If you don’t already have any compute options configured, just use the ‘Create new compute’ link highlighted below.
...
>*This sidebar displays when you select to create a new compute. If you need to worry about pricing, you can view  [this guide](https://azure.microsoft.com/en-us/pricing/details/machine-learning-service/) to determine what your selected VM size will cost you. If you leave ‘Minimum number of nodes’ at 0, you will not be charged when the compute is not in use, but you will lose data profiling capabilities.*
...
>*The AdventureWorksCustomerData.csv file has been uploaded using the highlighted ‘Upload’ button. Once uploaded, it is available to select (blue highlighted row).*
...
Configure which columns you think the algorithm should factor in by using the ‘Included’ toggle. For the Adventure Works data set, we are going to keep the following columns:
-   City
-   StateProvinceName
-   CountryRegionName
-   BirthDate
...
-   TotalChildren
-   YearlyIncome
-   BikeBuyer  _(our prediction column, or ‘label’)_
...
Next, select your prediction task and target column. In our case, we want to predict whether a customer will purchase a new bike. This makes it a binary  **Classification** task, as we are looking for an answer that will fit into either a ‘Yes’ or ‘No’ category. The target column will be  **BikeBuyer**.
...
If you don’t already have one, you will need to create a new notebook VM. Click  **New** and continue through the dialog.
...
A new notebook VM will show up in the list. Click through to  **JupyterLab**, which will take you to the Notebook Launcher screen. From here, click through to  **Python 3.6**  under the  **Notebook**  heading. Your brand new Jupyter notebook will open up, ready to rock and roll 😀
...
An example notebook is embedded below, which details the steps and code to connect to your model and send up a request to get back a prediction – feel free to review and copy-paste into your own environment as needed:
...
One last thing to note is that on the first run, you may see a message asking you to authenticate – you will need to navigate to the web address in the message, and input the code it provides you:
...
-   A proper data analysis phase – for example, we can do better than guesswork to decide which features to include
...
Consider though, that in this post we have configured, trained, deployed, and integrated with a classification model using Azure AutoML – pretty awesome, and not a bad place to call it a day.
